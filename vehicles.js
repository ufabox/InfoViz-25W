...
let activeVehicleFilters = new Set([
    'Cars & Taxis',
    'Motorcycles',
    'Buses & Minibuses',
    'Vans & Goods',
...
// Vehicles dashboard is intentionally initialized lazily.
// Reason: the page defaults to the Collisions tab, and most D3 charts
// measure container width on init (which is 0 when hidden).
let vehiclesInitialized = false;
function initVehiclesDashboard() {
    if (vehiclesInitialized) return;
...
document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', function () {
        ...
        // Lazy init when the Vehicles tab is opened
        if (tabId === 'vehicles') {
...
function drawVehicleStats() {
    ...
    d3.csv("data/vehicles_2024.csv").then(function (vehicles) {
...
// (Initialized via initVehiclesDashboard)
...
function drawImpactChart() {
    ...
    Promise.all([
        d3.csv("data/vehicles_2024.csv"),
        d3.csv("data/collisions_2024.csv")
...
function drawWeatherChart() {
    ...
    // Tooltip container (keep separate from the Collisions dashboard tooltips)
    let tooltip = d3.select("#tooltip-vehicles");
    if (tooltip.empty()) {
        tooltip = d3.select("body").append("div")
            .attr("id", "tooltip-vehicles")
            .style("position", "absolute")
            .style("visibility", "hidden")
            .style("background-color", "rgba(0, 0, 0, 0.8)")
            .style("color", "white")
...
function drawDistanceChart() {
    ...
    Promise.all([
        d3.csv("data/casualties_2024.csv"),
        d3.csv("data/collisions_2024.csv"),
        d3.csv("data/vehicles_2024.csv")
...
function drawAgeChart() {
    ...
    d3.csv("data/vehicles_2024.csv").then(function (data) {
